...
export type SolarSystemHandles = {
  ...
  /** 比例真相：true = 尺寸与距离 morph 到真实比例（1 AU = 55 单位） */
...
// 「比例真相」：真实比例下 1 AU 的场景单位。太阳半径 ≈ 0.26 单位，
// 地球半径 ≈ 0.0023 单位（一粒尘埃），海王星轨道 ≈ 1654 单位。
const AU_UNIT = 55;
const KM_PER_AU = 149.6e6;
const TRUE_CAM = new THREE.Vector3(0, 1500, 2500);
...
export function createSolarSystem(
  ...
  const starTex = track(new THREE.CanvasTexture(createStarCanvas()));
  {
    ...
    for (let i = 0; i < n; i++) {
      // 半径放到真实比例轨道之外（点大小不随距离衰减，观感不变）
      const r = 2600 + rand() * 3000;
  ...
  // ---------- 比例真相：可见性与拖尾重置 ----------
  // 真实比例下彗星椭圆/小行星带/拖尾都属于「压缩比例的艺术加工」，隐藏之
  const applyTrueScaleVisibility = () => {
    belt.visible = !trueScaleOn;
    cometNode.anchor.visible = !trueScaleOn;
    ionTail.points.visible = !trueScaleOn;
    dustTail.points.visible = !trueScaleOn;
    for (let i = 0; i < orbitLines.length; i++) {
      const isCometLine = i === orbitLines.length - 1;
      orbitLines[i].visible = orbitsVisible && !(trueScaleOn && isCometLine);
    }
    for (const t of trails) t.line.visible = orbitsVisible && !trueScaleOn;
  };
  ...
  const resetTrails = () => {
    ...
    for (const p of planets) {
      ...
      for (let i = 0; i < TRAIL_LEN; i++) {
        trail.positions[i * 3] = p.anchor.position.x;
        trail.positions[i * 3 + 1] = p.anchor.position.y;
        trail.positions[i * 3 + 2] = p.anchor.position.z;
      }
  ...
  const pickAt = (clientX: number, clientY: number): string | null => {
    if (trueScaleOn) return null; // 真实比例下行星只是尘埃，不可选中
  ...
  const frame = () => {
    ...
    scaleT += THREE.MathUtils.clamp((trueScaleOn ? 1 : 0) - scaleT, -dt / 4, dt / 4);
    const se = scaleT * scaleT * (3 - 2 * scaleT);
    ...
    for (let pi = 0; pi < planets.length; pi++) {
      ...
      if (!p.body.kind) {
        ...
        const orbitR = THREE.MathUtils.lerp(
          p.body.orbitRadius,
          p.body.realDistAU * AU_UNIT,
          se,
        );
        ...
        // 尺寸与轨道线随比例插值（真实半径按 AU 换算成场景单位）
        if (se > 0.0001) {
          const trueR = (p.body.realRadiusKm / KM_PER_AU) * AU_UNIT;
          p.anchor.scale.setScalar(THREE.MathUtils.lerp(1, trueR / p.body.radius, se));
          orbitLines[pi].scale.setScalar(orbitR / p.body.orbitRadius);
        } else if (p.anchor.scale.x !== 1) {
          p.anchor.scale.setScalar(1);
          orbitLines[pi].scale.setScalar(1);
        }
      }
    ...
    {
      const sunTrueR = (BODIES[0].realRadiusKm / KM_PER_AU) * AU_UNIT;
      sunAnchor.scale.setScalar(
        THREE.MathUtils.lerp(1, sunTrueR / BODIES[0].radius, se),
      );
    }
    ...
    // 行星拖尾：移动超过步长才追加，轨迹弧长与速度无关（真实比例下隐藏且不更新）
    if (scaleT < 0.01)
  ...
  return {
    ...
    setTrueScale(v) {
      ...
      applyTrueScaleVisibility();
      if (v) {
        // 先放开距离限制再飞出去，否则 OrbitControls 每帧会把相机拽回来
