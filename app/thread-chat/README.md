...
- **气泡输入框**（fork 首条消息策略「默认代拟 + 可选带问」，Phase A）：划选气泡内含可选输入框——留空提交 = 现状引导回复路径（接模型后为代拟首问 B）；输入后提交 = 问题成为分支首条 user 消息（C，`ForkInput.firstQuestion`）。Enter 提交 / Shift+Enter 换行 / ⌘Enter 提交且保留本列；按钮文案随输入切换（「开启分支讨论」↔「带着问题开分支」，⌘/override 文案优先）。
...
├── data.ts                   # 【演示数据】canned 内容——接真实模型时整体退役（§10）
├── mock-provider.ts          # 【演示数据】ReplyProvider 的 mock 实现：canned 内容流式吐出
...
- **气泡内轻量对话**（已调研，见 `docs/bubble-chat-research.md`）：划选后在气泡里直接输入并完成快速问答、可升格为完整分支——P1「气泡也是视口」的自然延伸。调研结论：气泡 = 树的第三种视口，首次提交输入即 fork（无 intro）、升格 = `openBranchUI` 换视口不换数据。**Phase A（气泡带输入框 + fork 首条消息策略）已落地**（见 §6.1、verify6）；Phase B（气泡内多轮 + 升格）待做，依赖 §10.1 流式改造。
